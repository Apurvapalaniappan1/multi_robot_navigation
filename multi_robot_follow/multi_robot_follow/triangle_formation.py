...
class TriangleFormation(Node):
    def __init__(self):
        ...
        self.phase = 'forming'
        self.group_goal = (0.2, -1.8)
        self.anchor_goal_tolerance = 0.12
        ...
        if self.phase == 'forming':
           ...
           if all_reached:
                self.get_logger().info(
                     'Triangle formation reached. Moving formation to goal.'
                )
                self.phase = 'moving'
            
        elif self.phase == 'moving':
            goal_x, goal_y = self.group_goal
            ...
            for robot in self.robots:
                ...
                cmd, _ = self.go_to_target(x, y, yaw, target_x, target_y)
                self.cmd_pubs[robot].publish(cmd)
            ...
            if anchor_distance < self.anchor_goal_tolerance:
                self.get_logger().info(
                    'Moving triangle reached group goal. Stopping robots.'
                )    
